            ...
            <div className="mx-auto max-w-7xl">
                <header className="mb-8 sm:mb-12 flex flex-col md:flex-row md:items-start justify-between gap-4 sm:gap-6">
                    ...
                    <div className="relative shrink-0 mx-auto md:mx-0">
                        <button className="peer flex items-center gap-2 text-slate-400 hover:text-white transition-colors bg-slate-900/50 hover:bg-slate-800 px-4 py-2 rounded-full border border-slate-700/50 text-sm font-medium backdrop-blur-sm">
                            <ShieldCheck className="w-4 h-4" />
                            Datenschutz
                        </button>

                        {/* Hover Overlay - Only visible when BUTTON is hovered (peer-hover) */}
                        <div className="absolute right-1/2 translate-x-1/2 md:translate-x-0 md:transform-none md:right-0 top-full mt-2 w-[calc(100vw-2rem)] max-w-[320px] sm:w-[320px] md:w-[350px] p-4 sm:p-6 bg-slate-900/95 border border-slate-700 rounded-xl shadow-2xl opacity-0 invisible peer-hover:opacity-100 peer-hover:visible transition-all duration-200 translate-y-2 peer-hover:translate-y-0 z-50 backdrop-blur-md pointer-events-none">
                            ...
                            <ul className="space-y-3 text-sm text-slate-300">
                                ...
                                <li className="flex gap-2 items-start"><Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" /> Kein Cookie-Banner nötig</li>
                                <li className="flex gap-2 items-start"><Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" /> Lokale Browser-Verarbeitung</li>
                                <li className="flex gap-2 items-start"><Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" /> Keine Speicherung oder Logging</li>
                                <li className="flex gap-2 items-start text-xs text-slate-500 mt-2 italic">Daten sind nach Reload weg.</li>
                            </ul>
                            ...
                            <div className="mt-5 pt-4 border-t border-slate-800 text-xs text-slate-500 flex justify-between items-center">
                                ...
                                <a href="https://github.com/shuhne/steuerlast-rechner.de" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-white transition-colors">
            ...
            <footer className="mx-auto max-w-7xl mt-12 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6 text-slate-500 text-sm">
                <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
                    ...
                    <nav className="flex gap-6">
                        ...
                        <button className="hover:text-slate-300 transition-colors">Datenschutz</button>
                    </nav>
                </div>
